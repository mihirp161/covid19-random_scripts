...
setwd('./BOT_csv_news10_streamAPI/')
...
dir.create("./batch1_2_restapi_data_botoresults_OLD/") #(Give path )
...
file.copy(files,
          to = "../batch1_2_restapi_data_botoresults_OLD/", recursive = T,
          overwrite = T, copy.mode = T, copy.date = F)
setwd('..')

#----------------------------------------------- rename all the files that were copied -----------------------------------------
#rename the files based on direcotry
setwd('./batch1_2_restapi_data_botoresults_OLD/')

folder<-"./batch1_2_restapi_data_botoresults_OLD/"
