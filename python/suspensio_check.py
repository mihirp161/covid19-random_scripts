...
def main(twitter_crendient, large_uids):
    ...
    auth = tweepy.OAuthHandler(twitter_crendient['FROM TWITTER'],
                               twitter_crendient['FROM TWITTER'])
    auth.set_access_token(twitter_crendient['FROM TWITTER'],
                          twitter_crendient['FROM TWITTER'])
    api = tweepy.API(auth, wait_on_rate_limit=True)
    
    large_uids= ['1184096866402590720']
    
    # Then, call check_status
    active_uids, inactive_user_status = check_status(api, large_uids)
    
    print("active uid: ", active_uids)
    print("inactive uid: ", inactive_user_status)
